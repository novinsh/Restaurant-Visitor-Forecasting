...
with open(wd + '/air_visit_data.csv') as f:
    ...
    for row in csvReader:
        try:
            ...
            month = get_month(visit_date)
            if visit_date in holidays:
                new_col = holidays[visit_date]
                row = row[:2] + [new_col] + row[2:] + [day_of_week, month]
            data.append(row)
...
data.insert(0, ['air_store_id', 'date', 'holiday', 'visitors', 'weekday', 'month'])

with open(wd + '/air_visit_with_month.csv', 'w') as g:
